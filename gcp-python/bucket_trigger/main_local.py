...
class Utilities:
    ...
    def list_bucket_data(project, bucket):
        ...
        for blob in bucket.list_blobs():
            ...
            elif content_name.endswith('.xlsx') or content_name.endswith('.xls'):
                list_excel.append(content_name)
    ...
    def geojson_into_bq(geojson_file):
        ...
        try:
            ...
            column_names_orig = list(geojson_geopandas.columns.values)
            parsed_field_names = Utilities.parse_column_names(column_names_orig)

            i = 0
            # Removes unwanted characters from field names
            for field_name in column_names_orig:
                field_name_parsed = parsed_field_names[i]
                geojson_geopandas.rename(columns={field_name: field_name_parsed}, inplace=True)
            ...
            print("BigQuery table could not be created from the Geojson file.")
            print(str(e))
            return
    ...
    def excel_into_bq(excel_file):
        ...
        client_bq = bigquery.Client()
        pd_excel_file = pd.ExcelFile(excel_file)
    ...
    def parse_csv_table(upload_uri):
        ...
        while rows_remain:
            with bucket_csv.open(upload_uri, 'r', errors='ignore') as f:
                ...
                if chunk_num == 1:
                    csv_contents = pd.read_csv(f, skiprows=skip_rows, nrows=Default.CSV_ROW_READ_LIMIT, low_memory=False)
                else:
                    csv_contents = pd.read_csv(f, skiprows=skip_rows + 1, nrows=Default.CSV_ROW_READ_LIMIT, low_memory=False, names=updated_column_names)
...
def data_added_to_bucket():
    ...
    for excel_file in list_excel:
        now = datetime.now()
        print('[' + str(now) + '] ' + "Excel: " + str(excel_file))

        output_table_name = os.path.basename(excel_file).replace('.geojson', '')
        upload_uri = 'gs://' + Default.BUCKET_TRIGGER + '/' + excel_file
        bq_table_uri = Default.PROJECT_ID + '.' + Default.BIGQUERY_DATASET_BUCKET + '.' + output_table_name

        Utilities.excel_into_bq(upload_uri)
    ...
    for geojson_file in list_geojson:
        now = datetime.now()
        print('[' + str(now) + '] ' + "Geojson: " + str(geojson_file))
