...
# get the package here and sends it to the cloud storage plugin ... 
...
class CloudStoragePlugin(plugins.SingletonPlugin):
    ...
    # mohab acted here
    plugins.implements(plugins.IPackageController, inherit=True) # get info about the pkg and sends it to ResourceCloudStorage
    ...
    def update_config(self, config):
        ...
        plugins.toolkit.add_resource('assets', 'cloudstorage_assets')
    ...
    def configure(self, config):
        ...
        for rk in required_keys:
            if config.get(rk) is None:
                raise RuntimeError('Required configuration option {0} not found.'.format(rk))
    ...
    def get_resource_uploader(self, data_dict):
        ...
        return storage.ResourceCloudStorage(data_dict,pack)
