"""CKAN CLI commands for the wro extension"""
import datetime
...
_PYCSW_MATERIALIZED_VIEW_NAME = "public.wro_pycsw_view"


@click.group()
@click.option("--verbose", is_flag=True)
def wro(verbose: bool):
    """Commands related to the wro extension."""
...
@wro.group()
def delete_data():
    """Delete wro bootstrapped and sample data"""
...
@bootstrap.command()
def create_wrt_organizations():
    """Create main wro organizations

    This command creates the main wro organizations.

    This function may fail if the wro organizations already exist but are disabled,
    ...
    existing_organizations = toolkit.get_action("organization_list")(
        context={},
        data_dict={
            "organizations": [org.name for org in WRO_ORGANIZATIONS],
            "all_fields": False,
        },
    )
    user = toolkit.get_action("get_site_user")({"ignore_auth": True}, {})
    for org_details in WRO_ORGANIZATIONS:
...
@delete_data.command()
def delete_wro_organizations():
    """Delete the main wro organizations.

    This command will delete the wro organizations from the CKAN DB - CKAN refers to
...
def defer_to_alembic(alembic_command, collect_args, command_arg, command_kwarg):
    ...
    alembic_wrapper = AlembicWrapper("wro")
    bool_keys = (
        "verbose",
        "autogenerate",
    )
...
def refresh_pycsw_materialized_view(ctx, post_run_delay_seconds: int):
    ...
    `launch-ckan-cli wro pycsw refresh-materizalied-view`
