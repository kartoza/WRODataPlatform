...
test('test elements on about page', async ({ page }) => {
  ...
  await expect(page.getByText('Welcome to the wro Portal')).toBeVisible();

  await expect(page.getByRole('link', { name: 'METADATA', exact: true })).toBeVisible();

  await expect(page.getByRole('heading')).toContainText('Discover a world of data-driven possibilities at the wro-Portal, where information converges to empower data sharing and decision making.');
  ...
  await expect(page.locator('#title-header')).toContainText('The wro-Portal is the result of a pioneering collaboration between SANSA (South African National Space Agency) and SAEON (South African Earth Observation Network). United by a common vision, the wro-Portal platform was conceived to revolutionize data sharing among diverse entities, fostering synergy and knowledge exchange.');
  ...
  await expect(page.getByText('At the wro-Portal, we')).toBeVisible();
